package com.kunluntop.eportal.utils.http;
...
public class HttpClientHelper {
	...
	paraMap.put("user", "fkk");
		String string=	sendPost("http://localhost:80/",paraMap,"utf-8");
    ...
    /**
     * @Description:使用HttpClient发送post请求       Apache
     * @author:liuyc
    ...
    public static String httpClientPost(String urlParam, Map<String, Object> params, String charset) {
        ...
        while (iterator.hasNext()) {
            ...
            list.add(new BasicNameValuePair(elem.getKey(), String.valueOf(elem.getValue())));  //封装参数固定格式
        }
        BufferedReader br = null;
        try {
            if (list.size() > 0) {
            ...
            // 读取服务器响应数据
            resultBuffer = new StringBuffer();
            br = new BufferedReader(new InputStreamReader(response.getEntity().getContent())); //读取相应报文的实体内容
            String temp;
            while ((temp = br.readLine()) != null) {
                resultBuffer.append(temp);
            }
    ...
    /**
     * @Description:使用HttpClient发送get请求  Apache
     * @author:liuyc
    ...
    public static String httpClientGet(String urlParam, Map<String, Object> params, String charset) {
        ...
        if (params != null && params.size() > 0) {
            for (Entry<String, Object> entry : params.entrySet()) {
                ...
                sbParams.append("=");
                try {
                    sbParams.append(URLEncoder.encode(String.valueOf(entry.getValue()), charset));//URL 编码
    ...
    public static String sendSocketPost(String urlParam, Map<String, Object> params, String charset) {
        ...
        try {
            ...
            osw.write(sb.toString());//发送请求
            osw.flush();
            is = socket.getInputStream(); //接受响应
            String line = null;
            // 服务器响应体数据长度
            int contentLength = 0;
            // 读取http响应头部信息
    ...
    public static String sendSocketGet(String urlParam, Map<String, Object> params, String charset) {
        ...
        try {
            ...
            osw.write(sb.toString()); //发送请求
            osw.flush();
            is = socket.getInputStream(); //接受请求响应
            String line = null;
            // 服务器响应体数据长度
            int contentLength = 0;
            // 读取http响应头部信息
